    ...
    <div class="main-body" :class="playPlatform" :style="{'-webkit-overflow-scrolling': scrollMode}">
      <mt-loadmore :top-method="loadTop" :auto-fill="false" ref="loadmore">
        <div style="min-height:11rem;margin-bottom: 0.5rem;position: relative" class="table">
          ...
          <div class="numberNo clear" v-for="(item,index) in bonuscode"
               :style="index==0?{'borderTop':'1px solid #c0c0c0'}:{'borderTop':''}">
            <p class="type timesNumber">
              <span>{{item.issue.substring(4,item.issue.length)}}</span>
            </p>
            <p class="no code" :style="{width: 85/vaildnum.length + '%'}" v-for="item in vaildnum"><span
              class="ballNumber" v-if="item == wei[index]">{{item}}</span></p>
          </div>
        </div>
      </mt-loadmore>
    </div>
