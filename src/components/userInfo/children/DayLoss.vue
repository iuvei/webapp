              ...
              <table class="record_table" width="100%" border="0" cellspacing="0" cellpadding="2">
                ...
                <tbody>
                  <tr align="center" v-for="(item,index) in data.tableRows" style=" height: 1rem;">
                    ...
                    <td v-tap="{ methods: _checkDetails, item: item }"  v-text="item.buttons[0].text" style="text-decoration: underline; color: #10aeff;"></td>
                  </tr>
                </tbody>
              </table>
            </div>
...
//      this._getSecondaryAgent()
    ...
    methods: {
      ...
      _checkDetails(params) {
        this.$router.push({path: '/userInfo/DayLossBrokerage', query:{userid: params.item.userid, gmt_sale: params.item.gmt_sale, un_click: true}})
      },
      loadTop () { //  组件提供的下拉触发方法
        this.postData.p = 1
        this._getSecondaryAgent()
