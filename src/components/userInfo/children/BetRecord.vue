  ...
  <div>
    ...
    <transition name="slideDown">
      <div class="main" v-show="show">
        <dateSelectView :starttime="getData.starttime" :endtime="getData.endtime" @getStartTime="onGetStartTime"
                        @getEndtime="onGetEndtime"></dateSelectView>
        <div class="inputBox clear">
          <p class="searchtit" :class="{playPlatform}">展示方式</p>
          <p class="caizhong">
        ...
        <div class="inputBox" :class="{playPlatform}">
          <p class="searchtit">中奖状态</p>
          <div class="select">
            <ul class="selectBox clear">
              <li class="left" :class="{activeStatus : isActiveStatus === index}"
                  v-tap="{ methods:showname, item: item, index: index }" v-for="(item,index) in winningState"
                  :key="index" v-text="item.name"></li>
            </ul>
          </div>
        </div>
      </div>
          ...
          <table class="record_table" width="100%" border="0" cellspacing="0" cellpadding="2">
            ...
            <tr v-tap="{ methods:_betRecordDetails, item: item }" v-for="(item,index) in dataList" :key="index"
              ...
              <td style="position: relative;">
                ...
                <span style="color:#c7202c;"
                      v-if="item.iscancel==0&&item.isgetprize==1&&item.prizestatus != 0">已派奖</span>
