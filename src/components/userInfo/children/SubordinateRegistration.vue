    ...
    <div class="header_app_v">
      ...
      <form @submit.prevent="submit">
        <ul class="registerBox">
          ...
          <li>
            <p class="linkL">用户名</p>
            <p><input type="text" name="username" v-model="postData.username" @blur="checkUserName('usercheck','username', postData.username)" placeholder="期待创建的下级账号"></p>
          </li>
          <li class="tips">由字母或数字组成的6-16个字符,不能连续四位相同的字符,首字不能以
  0或者o开头</li>
          <li>
            <p class="linkL">昵称</p>
            <p><input type="text" name="nickname" v-model="postData.nickname" @blur="checkNickName('nicknamecheck','nickname', postData.nickname)" placeholder="请输入昵称"></p>
            <br class="clear">
          </li>
          <li class="tips">
            2-6位字符组成
          ...
          <li>
            <p class="linkL">密码</p>
            <p><input type="password" name="userpass" v-model="postData.userpass" @blur="checkUserPass('checkpwd','pwd', postData.userpass)"placeholder="下级密码"></p>
            <br class="clear">
          </li>
          <li class="tips">
          ...
          <li>
            ...
            <p  class="select_number">
              <span v-tap="{ methods:Less}" :class="{forbidColor: postData.keeppoint<=0.1}" :style="{borderRight:0}">-</span>
              <input type="text" name="keeppoint" @blur="_blurKeeppoint" v-model="postData.keeppoint">
              <span v-tap="{ methods:plus}" :class="{forbidColor: postData.keeppoint>=parseFloat(maxPoint)}" :style="{borderLeft:0}">+</span>
            </p>
          </li>
          <li class="tips">
        ...
        <div class="seestyle">
          <div v-if="AllTypeAccNumData.length!==0">
            <p class="seepeie" v-tap="{ methods:_peie}" >查看配额 <i id="peie_icon_hook" class="icon_hook" style="left: 1.6rem;"></i></p>
          ...
          <div v-if="AllLotterysflag.length!==0">
            <p class="seeRebate" v-tap="{ methods:_fandian}">查看分彩种返点<i id="icon_hook" class="icon_hook" style="left: 2.4rem;"></i></p>
            <transition name="slide-fade">
