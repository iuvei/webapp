      ...
      <ul class="rechargeBox">
        ...
        <li>
          ...
          <ul v-if="isshow" class="clear">
            <li v-tap="{ methods: addred, item: item}" :class="{active: index==0}" v-for='(item,index) in third'><img :src='item.imgurl' /></li>
          </ul>
        </li>
        <li>
          ...
          <ul class="clear" v-if="show">
            <li v-tap="{ methods: addred, item: item}" :class="{active: index==0 && third.length == 0}"  v-for='(item,index) in bankList'><img :src='item.imgurl' /></li>
          </ul>
        </li>
      </ul>
      <div class="tips">最大充值金额<i>{{loadmax}}</i>元,最小可充值金额<i>{{loadmin}}</i>元</div>
      ...
      methods: {
        ...
        nextAction(){
          ...
          this.$vux.loading.show({
        	...
        	let obj = {tag:tempInfo['actioner'].split('=')[1],flag:'load',username:this.username,
        	bid:tempInfo.tbank_id,amount:this.inputMoney,Ruleset:'deposit-hcp',alipayName:this.alplayname,'play_source':5}
  				this.httpAction(pwdurl,(res)=>{
