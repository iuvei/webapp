    ...
    <div class="wage_table_1">
      <div style="position: relative">
        <ul class="table_thead table_dividend thead_hook clear">
          ...
          <li class="dropdown" v-tap="{ methods: _clickDropdown }" style="background:none; z-index: 999;"
              v-if="this.$route.query.dropclick">
      ...
      <ul class="table_dividend table_body clear" v-for="(row, index) in datas.tableRows">
        ...
        <li v-if="indx < thead_2"
            ...
            v-tap="{ methods:_clickUserName, row: row , item: item, index: index , indx: indx}">{{row[item] |
          numberFilter(item)}}
        </li>
      </ul>
    </div>
        ...
        <li v-if="indx >= thead_2"
            ...
            :class="{
                     ...
                     buttons_color:
                        ...
                        item == 'buttons_daywage',
                     profit_lose:
                      indx == datas.gridColumns.length - 1 && parseFloat(row['profit']) < 0,
                      profit_win:
                        indx == datas.gridColumns.length - 1 && parseFloat(row['profit']) > 0,
                        unbuttons:
                        row[item] == true&&playPlatform==='web'
                  }"
            v-tap="{ methods:_clickUserName, row: row , item: item, index: index , indx: indx}">{{row[item] |
          numberFilter(item)}}
        </li>
    ...
    methods: {
          ...
          if (params.item == 'rdate' || params.item == 'date_range') { // 团队报表日期
            this.$router.push({
              ...
              query: {
                ...
                start_date: this.startAndEndtime.start_date,
                end_date: this.startAndEndtime.end_date
              }
            })
          }
        ...
        if (params.item === 'username') { // 点击用户名
          if (this.$route.path == '/userInfo/DayWageClickUser') {
            this.$router.push({
              ...
              query: {parent_id: params.row.userid, username: params.row.username}
            })
      ...
      _getSecondaryAgent(val) {
        ...
        this.$router.push({
          ...
          query: {'username': username, 'gmt_sale': this.$store.state.starttime, userid: val.userid, 'agree': agree}
        })
      }
    },
    filters: {
